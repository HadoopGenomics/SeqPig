...
SeqPig is based on Hadoop-BAM, which among other things provides IO access
to BAM files from within Hadoop. Hadoop-BAM provides access to the data
on the bases of SAMRecords (see \url{http://picard.sourceforge.net/javadoc/net/sf/samtools/SAMRecord.html}),
...
\end{lstlisting}
For more details on available filters consider looking at file {\tt scripts/filter\_defs.pig}.
...
\end{lstlisting}
{\bf NOTE}: this is roughly equivalent to executing from the command line:
\begin{lstlisting}
...
\end{lstlisting}
{\bf NOTE}: this is equivalent to executing from the command line:
\begin{lstlisting}
...
parameters that determien the size of the bins ({\tt binsize}) and the
maximum number of reads considered per bin ({\tt reads\_cutoff}), wich is
similar to the maximum depth parameter that samtools accepts. Note,
however, since it is set on a per-bin level you may chose it dependent on
the read length and bin size, as well as the amount of memory available to
the nodes.
...
...
\end{lstlisting}
{\bf NOTE}: this example script is equivalent to executing from the command line:
\begin{lstlisting}
...
depends on the choice of command line parameters. If you are in need for
performance tuning, consider the following Hadoop specific parameters in {\tt mapred-site.xml}:
\begin{itemize}
\item {\tt mapred.tasktracker.map.tasks.maximum}: \\the maximum number of mappers per worker node
\item {\tt mapred.tasktracker.reduce.tasks.maximum}:\\ the maximum number of reducers per worker node
\item {\tt mapred.child.java.opts}:\\ Java options passed to child virtual machines at the worker nodes; this may contain for example {\tt -Xmx1000M}
for setting the maximum Heap size to 1GB, depending on the cluster resources
\end{itemize}
Aditionally, it is possible to pass command line parameters (such as mapper and reducer memory limits) along.
Consider for example the pig call (see {\tt tools/tun\_all\_pileup2.sh})
\begin{lstlisting}
...
\end{lstlisting}
By setting appropriate values for {\tt MAP\_MEMORY, REDUCE\_MEMORY, CHILD\_MEMORY} and for the number of available reduce slots {\tt REDUCESLOTS} different performance
can be achieved.
